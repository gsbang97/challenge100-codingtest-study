...
for _ in range(t):
    ...
    for cmd in input().split():
        if cmd == 'L-':
            left = list(list(map(list, zip(*left)))[::-1])
            for i in range(3):
            ...
            right = list(list(map(list, zip(*right)))[::-1])
            for i in range(3):
            ...
            back = list(list(map(list, zip(*back)))[::-1])
            u = list(map(list, zip(*left[::-1])))
            l = list(map(list, zip(*down[::-1])))
            d = list(map(list, zip(*right[::-1])))
            r = list(map(list, zip(*up[::-1])))
